...
/**
 * These cover the two things the properties records are for: the processor must refuse to start when an outbound
 * address is missing, instead of starting healthy and failing during a batch, and the property names must keep binding
 * exactly as the deployment spells them.
 *
 * <p>
...
class ConfigurationPropertiesTest {
    ...
    private static String[] shippedConfig() {
        return new String[] { "operations-app.contactpoint=https://ops-bk.mifos.gazelle.test",
            ...
            "operations-app.endpoints.batch-summary=/api/v1/batch", "operations-app.endpoints.batch-aggregate=/api/v1/batch/detail",
    ...
    void urlsAreComposedTheWayTheOldPostConstructDidIt() {
        runner.withPropertyValues(shippedConfig()).run(context -> {
            ...
            assertThat(properties.batchTransactionUrl()).isEqualTo("https://ops-bk.mifos.gazelle.test/api/v1/batch/transactions");
            assertThat(properties.batchSummaryUrl()).isEqualTo("https://ops-bk.mifos.gazelle.test/api/v1/batch");
            assertThat(properties.batchAggregateUrl()).isEqualTo("https://ops-bk.mifos.gazelle.test/api/v1/batch/detail");
            assertThat(properties.authUrl()).isEqualTo("https://ops-bk.mifos.gazelle.test/oauth/token");
        });
    }
